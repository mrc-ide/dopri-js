...
describe("Integrate function", () => {
    ...
    it("works for dopri, preventing use with incorrect output", () => {
        ...
        expect(() => integrate(rhs, [1], 0, 3, {}, out)).toThrow("Can't used delayed output with non-delayed rhs");
    });
    ...
    it("works for dde, preventing use with incorrect output", () => {
        ...
        const out = (t: number, y: number[]) => [y[0] + 1];
        expect(() => integrate(rhs, [1], 0, 3, {}, out)).toThrow("Can't used non-delayed output with delayed rhs");
    });
});
